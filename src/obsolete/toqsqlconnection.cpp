...
public:
    ...
    class qSqlQuery : public queryImpl
    {
        ...
        std::list<QString> extraData(const toQSqlProviderAggregate &aggr)
        {
            ...
            const QList<toCache::CacheEntry const*> &objects = query()->connection().objects(false);
            for (QList<toCache::CacheEntry const*>::const_iterator i = objects.begin(); i != objects.end(); i++)
            {
                ...
                else if ((*i)->type == toCache::TABLE)
                {
                    if (aggr.Type == toQSqlProviderAggregate::AllTables ||
                        ...
                        toPush(ret, (*i)->name.first + "." + (*i)->name.second);
                }
            }
            return ret;
        }
    ...
    public:
        ...
        /*virtual*/ QList<toCache::CacheEntry*> objectNamesNew(QString const& schema)
        {
            QList<toCache::CacheEntry*> ret;

            try
            {
                toQuery databases(connection(), SQLListDatabases, schema);
                while (!databases.eof())
                {
                    QString db = databases.readValue();
                    ret.push_back(toCache::createCacheEntry(db, "", "DATABASE", ""));
                    try
                    {
                        toQuery tables(connection(), SQLListObjectsDatabase, db, "");
                        while (!tables.eof())
                        {
                            QString Name = tables.readValue();
                            QString Owner = db;
                            QString Type = QString::fromLatin1("TABLE");
                            for (int i = 2; i < tables.columns(); i++) /// I just wan't the last column except for the first one
                                tables.readValue();
                            QString Comment = tables.readValue();
            ...
            catch (...)
            {
                ...
                while (!tables.eof())
                {
                    ...
                    if (tables.columns() > 1)
                        Owner = tables.readValue();
                    else
                        Owner = connection().database();
                    if (tables.columns() > 2)
                        Type = tables.readValue();
                    else
                        Type = QString::fromLatin1("TABLE");
                    ret.push_back(toCache::createCacheEntry(Owner, Name, Type, ""));
                }
            }
        ...
        virtual void describeObject(toCache::CacheEntry*, toConnectionSub *conn)
        {
        };
