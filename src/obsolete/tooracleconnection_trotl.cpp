...
static toSQL SQLListObjectsInDatabase("toOracleConnection:ListObjectsInDatabase",
                                      "select a.owner,a.object_name,a.object_type,b.comments\n"
                                      "  from sys.all_objects a,\n"
                                      "       sys.all_tab_comments b\n"
                                      " where a.owner = b.owner(+) and a.object_name = b.table_name(+)\n"
...
static toSQL SQLListObjectInSchema("toOracleConnection:ListObjectsInSchema",
                                   "select a.owner,a.object_name,a.object_type,b.comments\n"
                                   "  from sys.all_objects a,\n"
                                   "       sys.all_tab_comments b\n"
                                   " where a.owner = b.owner(+) and a.object_name = b.table_name(+)\n"
...
public:
    ...
    public:
        ...
        {
            TLOG(0, toDecorator, __HERE__)
                    << "Throwing an exception - what:" << exc.what() << std::endl
                    << exc.get_sql() << std::endl
                    << "--------------------------------------------------------------------------------"
                    << std::endl;
            ThrowException(exc);
    ...
    public:
        ...
        virtual toQColumnDescriptionList describe(void)
        {
            ...
            for(; it != col.end(); ++it)
            {
                TLOG(0, toDecorator, __HERE__) << "Var: " << (*it)->typeName() << std::endl;
                toCacheNew::ColumnDescription desc;
    ...
    class oracleConnectionImpl : public toConnection::connectionImpl
    {
        ::trotl::OciEnv &_env;
        QString connectString(void);
        std::list<QString> RowIDs;

        static oracleSub *oracleConv(toConnectionSub *sub)
        {
    ...
    public:
        oracleConnectionImpl(toConnection *conn, ::trotl::OciEnv &env) :
            toConnection::connectionImpl(conn), _env(env)
        {
            RowIDs.push_back("ROWID");
            RowIDs.push_back("ORA_ROWSCN");
        }
...
///                                                                        const QString &name)
///     {
///         QList<toConnection::objectName> ret;
///
...
///     }
        /*virtual*/
        QList<toCacheNew::CacheEntry*> objectNamesNew(QString const& schema)
        {
            QList<toCacheNew::CacheEntry*> ret;
            std::list<toQValue> par;

            if(schema.isEmpty())
                try
                {
                    //toQuery objects(connection(), toQuery::Long, SQLListObjectsInDatabase, par);
                    toQuery objects(connection(), toQuery::Long, toSQL::sql("toOracleConnection:ListObjectsInDatabase", connection()), par);
                    while (!objects.eof())
                    {
                        ...
                        toCacheNew::CacheEntry *e = toCacheNew::createCacheEntry(owner, name, type, comment);
                        if(e)
                            ret.append(e);
                    }
                }
                ...
                {
                    ThrowException(exc);
                    return QList<toCacheNew::CacheEntry*>();
                }
            return ret;
        };

        virtual void describeObject(toCacheNew::CacheEntry* e, toConnectionSub *sub)
        {
            QString objectName = e->name.first + '.' + e->name.second;
            oracleSub *s = oracleConnectionImpl::oracleConv(sub);
            ::trotl::Describe *d = ::trotl::Describe::createDescription(*s->_conn, objectName.toStdString());

            e->details = QString::fromUtf8(d->toString().c_str());
            e->described = true;
        };
...
///
    ...
    {
        return new oracleConnectionImpl(conn, *_envp);
    }
...
QString toOracleProvider::oracleConnectionImpl::connectString(void)
{
    QString ret;
...
toConnectionSub* toOracleProvider::oracleConnectionImpl::createConnection(void)
{
...
/*virtual*/ QString toOracleProvider::oracleConnectionImpl::quote(const QString &name, const bool quoteLowercase)
{
    bool ok = true;
...
/*virtual*/ QString toOracleProvider::oracleConnectionImpl::unQuote(const QString &str)
{
