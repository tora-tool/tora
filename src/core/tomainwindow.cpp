...
namespace ToConfiguration
{
    QVariant Main::defaultValue(int option) const
    {
        ...
        {
            case LastVersion:
                return QVariant(QString("")); // TORAVERSION
            case FirstInstall:
                return QVariant(QString("")); // QDateTime::currentDateTime().toString()
            case RecentFiles:
                return QVariant(QStringList());
            case RecentMax:
                return QVariant((int)25);
            case LastDir:
    ...
    QVariant Main::toraIIValue(int option) const
    {
        ...
        switch (option)
        {
            case LastVersion:
                ...
                retval = s.value("LastVersion");
                break;
            case FirstInstall:
                s.beginGroup("preferences");
                retval = s.value("FirstInstall");
                break;
            case RecentFiles:
                s.beginGroup("preferences");
                retval = s.value("RecentFiles");
                break;
            case RecentMax:
                s.beginGroup("preferences");
                retval = s.value("RecentMax");
                break;
            case LastDir:
                s.beginGroup("preferences");
                retval = s.value("LastDir");
                break;
//	case Encoding:      return QVariant(QString("Default"));
            case DefaultTool:
                s.beginGroup("preferences");
                return s.value("DefaultTool");
                break;
...
//	case ToolsMap:
            case MainWindowGeometry:
                ...
                retval = s.value("geometry");
                break;
            case MainWindowState:
                s.beginGroup("toMainWindow");
                retval = s.value("state");
                break;
            case LeftDockbarState:
                s.beginGroup("toMainWindow");
                retval = s.value("leftDockbar");
                break;
            case RightDockbarState:
                s.beginGroup("toMainWindow");
                retval = s.value("rightDockbar");
                break;
            default:
                s.beginGroup("preferences");
                retval = QVariant();
