...
public:
     ...
     * @param buf String to set as first column
     */
...
private:
    ...
    //// TODO: whole toResultView must be rewritten - simplified
    class QueryWrapper {
    public:
    	QueryWrapper(toConnection &conn, const QString &sql, toQueryParams const &params)
    		: c(conn)
    		, q(c, sql, params)
    	{}

    	bool eof(void) { return q.eof(); };
    	inline unsigned columns(void) const { return q.columns(); };
    	toQValue readValue(void) {return q.readValue(); };
    	toQColumnDescriptionList describe(void) { return q.describe(); };
    private:
    	toConnectionSubLoan c;
    	toQuery q;
    };
...
public slots:
     ...
     */
    virtual void setFilter(toResultFilter *filter)
    {
        Filter = filter;
    }
