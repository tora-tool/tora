...
toTreeWidgetItem *toTreeWidget::toFindItem(const QString &str)
{
...
toTreeWidgetItem *toTreeWidget::FindItem(toTreeWidgetItem *first, const QString &str)
{
    while (first)
    {
        ...
        else
        {
            ...
            {
                toTreeWidgetItem *ret = FindItem(first->firstChild(), str.mid(tmp.length()));
                if (ret)
                    return ret;
            }
        }
