...
static QString FormatTime(double tim)
{
    QString timstr;
    if (tim >= 3600)
        timstr = QString::asprintf("%d:%02d:%02d.%02d",
                       int(tim) / 3600,
                       (int(tim) / 60) % 60,
                       int(tim) % 60,
                       int(tim * 100) % 100);
    else if (tim > 1)
        timstr = QString::asprintf("%d:%02d.%02d",
                       (int(tim) / 60) % 60,
                       int(tim) % 60,
                       int(tim * 100) % 100);
    else if (tim > 0.001)
        timstr = QString::asprintf("%d ms", (int)(tim * 1000));
    else if (tim > 0)
        timstr = QString::asprintf("%d ns", (int)(tim * 1E9));
    else
        timstr = "0";
    return timstr;
}
...
{
    QToolBar *toolbar = Utils::toAllocBar(this, tr("PL/SQL Profiler"));
...
void toProfiler::changeRun(void)
{
    ...
    try
    {
        toQList vals = toQuery::readQuery(connection(), SQLTotalTime, toQueryParams() << run);
        Units->setTotal(Utils::toShift(vals).toDouble());
        Units->refreshWithParams(toQueryParams() << run);
        Info->refreshWithParams(toQueryParams() << run);
    }
    TOCATCH
}
