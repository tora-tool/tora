    ...
    : QAbstractItemModel(parent)
{
    ...
    toQuery typelst(conn, SQLObjectPrivs, toQueryParams());
    QString key;
    QStringList value;
    while (!typelst.eof())
    {
...
void toSecurityTreeModel::setupModelData(const QString & user)
{
    ...
    toConnectionSubLoan conn(toConnection::currentConnection(this));
    toQuery grant(conn, SQLObjectGrant, toQueryParams() << user);
    QString yes = "YES";
    ...
    const QList<toCache::CacheEntry const*> objectList = toConnection::currentConnection(this).getCache().entries(true);
    for (QList<toCache::CacheEntry const*>::const_iterator i = objectList.begin()
            ; i != objectList.end()
            ; i++)
    {
        type = (*i)->type;
