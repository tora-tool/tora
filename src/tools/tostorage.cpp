...
toDropTablespace::toDropTablespace(QWidget* parent, const char* name)
    : QWidget(parent)
{
    setupUi(this);
    if (!name)
...
std::list<QString> toDropTablespace::sql()
{
    ...
    {
        Utils::toPush(ret, QString::fromLatin1("INCLUDING CONTENTS "));
        if (dataFileCheck->isChecked())
            Utils::toPush(ret, QString::fromLatin1("AND DATAFILES "));
        if (CascadeCheck->isChecked())
            Utils::toPush(ret, QString::fromLatin1("CASCADE CONSTRAINTS "));
    }
    else
        Utils::toPush(ret, QString::fromLatin1(""));
    return ret;
}



toStorageTablespace::toStorageTablespace(QWidget* parent, const char* name)
    : QWidget(parent)
{
    setupUi(this);
...
void toStorageTablespace::permanentToggle(bool val)
{
    try
    {
        if (toConnection::currentConnection(this).version() >= "08")
        {
...
std::list<QString> toStorageTablespace::sql()
{
    ...
    if (ExtentGroup->isEnabled() && Dictionary->isChecked())
        Utils::toPush(ret, QString::fromLatin1("MINIMUM EXTENT ") + MinimumExtent->sizeString());
...
toStorageDatafile::toStorageDatafile(bool temp, bool dispName, QWidget* parent, const char* name)
    : QWidget(parent),
      Tempfile(temp)
{
    ...
    : QDialog(parent)//, "Storage Dialog", true)
{
    ...
    if (!tablespace.isNull())
    {
        ...
        if (!drop)
        {
            ...
            {
                toQList result;
                result = toQuery::readQuery(toConnection::currentConnection(this), SQLTablespaceInfo, toQueryParams() << tablespace);
                if (result.size() != 10)
    ...
    : QDialog(parent)//, "Storage Dialog", true)
{
    ...
    try
    {
        ...
        toQList result = toQuery::readQuery(conn, SQLTablespaceInfo, toQueryParams() << tablespace);
        if (result.size() != 10)
            throw tr("Invalid response from query");
        Tablespace->MinimumExtent->setValue(Utils::toShift(result).toInt());

        TablespaceOrig = tablespace;
        Tablespace->Modify = true;
        Default->setEnabled(false);
        if ( (QString)Utils::toShift(result) == "DICTIONARY")
        ...
        Tablespace->Permanent->setChecked( (QString)Utils::toShift(result) == "PERMANENT");
        Tablespace->Logging->setChecked( (QString)Utils::toShift(result) == "LOGGING");
        Tablespace->Online->setChecked( (QString)Utils::toShift(result) == "ONLINE");
        ...
        Default->InitialSize->setValue(Utils::toShift(result).toInt());
        Default->NextSize->setValue(Utils::toShift(result).toInt());
        Default->InitialExtent->setValue(Utils::toShift(result).toInt());
        int num = Utils::toShift(result).toInt();
        if (num == 0)
            Default->UnlimitedExtent->setChecked(true);
        else
            Default->MaximumExtent->setValue(num);
        Default->PCTIncrease->setValue(Utils::toShift(result).toInt());
    }
    ...
    : QDialog(parent)//, "Storage Dialog", true)
{
    ...
    try
    {
        ...
        result = toQuery::readQuery(conn, SQLDatafileInfo, toQueryParams() << tablespace << filename);
...
void toStorage::slotWindowActivated(toToolWidget* widget)
{
    if (!widget)
        return;
    if (widget == this)
