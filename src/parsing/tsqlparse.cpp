...
namespace SQLParser
{
	...
	TSQLPARSER_EXPORT QMap<QPair<QString, QString>, ObjectCache::TableInfo>::const_iterator ObjectCache::getTable(QString const& currentSchema, QString const& schema, QString const& table) const
	{
		...
		if( schema.isEmpty())
		{
			...
			retval = _mTableMap.constFind(QPair<QString, QString>(currentSchema, table));
			if( retval != _mTableMap.end())
				return retval;

			// 1st try (b) - schema is empty => "table" is local synonym in my currentSchema
			synonymTranslationNew = _mSynonymMap.constFind(QPair<QString,QString>(currentSchema, table));
			...
			if( synonymTranslationNew == _mSynonymMap.end()) 
				synonymTranslationNew = _mSynonymMap.constFind(QPair<QString, QString>(QString("PUBLIC"), table));
			...
			if( synonymTranslationNew != _mSynonymMap.end())
			{
				...
				do {
					...
					if( synonymTranslationNew == _mSynonymMap.end())
						...
						synonymTranslationNew = _mSynonymMap.constFind(QPair<QString,QString>(QString("PUBLIC"), synonymTranslationLast->second));
			...
			QMap<QPair<QString, QString>, ObjectCache::TableInfo>::const_iterator retval;
			retval = _mTableMap.constFind(QPair<QString, QString>(schema, table));
			if( retval != _mTableMap.end())
				return retval;
			...
			QSet<QPair<QString, QString> > loopStack;
			synonymTranslationNew = _mSynonymMap.constFind(QPair<QString,QString>(schema, table));
	...
	TSQLPARSER_EXPORT Token const* Statement::translateAlias(QString const& alias, Token const *context)
	{
	...
	TSQLPARSER_EXPORT Token const* Statement::getTableRef(QString const& tableName, Token const *context)
	{
		for( SQLParser::Statement::token_const_iterator_to_root k(context); k->parent(); ++k)
		{
			if( k->getTokenType() == Token::S_SUBQUERY_NESTED ||
	...
	TSQLPARSER_EXPORT  void Statement::token_const_iterator::increment()
	{
  	...
  	TSQLPARSER_EXPORT  void Statement::token_const_iterator::decrement()
	{
	...
	TSQLPARSER_EXPORT void Statement::token_iterator_to_root::increment()
	{
		if(m_token)
			m_token = m_token->parent();
	};

	TSQLPARSER_EXPORT bool Statement::token_iterator_to_root::equal(Statement::token_iterator_to_root const& other) const
	{
		return this->m_token == other.m_token;
	};
