...
QString toGetToken(const QString &str,int &pos,bool forward)
{
  QChar c;
  QChar nc;
  QChar endString;
  ...
  while(pos<int(str.length())&&pos>=0) {
    if (!forward)
      pos--;
    c=str[pos];
    if ((forward&&pos<int(str.length()-1))||(!forward&&pos>0))
      nc=str[pos+inc];
    else
      nc='\n';
    if (forward)
    ...
    if (state==space) {
      if(forward&&c=='-'&&nc=='-') {
	while(pos>=0&&pos<int(str.length())) {
	  if (str[pos]=='\n')
	    break;
	  pos+=pos;
	}
