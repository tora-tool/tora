...
#if (defined(OTL_ORA8I)||defined(OTL_ORA9I)||defined(OTL_ORA10G)) && defined(OTL_ODBC)
...
public:
    ...
    void remove
        (const otl_stream_shell_generic* s, const OTL_STRING_CONTAINER& stmtxt)
    {
#ifdef OTL_ACE
        ace_map_entry* cur = 0;
    ...
    void add
        (otl_stream_shell_generic* s, const char* stm_text)
    {
        ...
        { // entry not found
            ...
            { // erase the least used entry and add new one
                ...
                for (sc_type::iterator i = elem0;i != elemN;++i)
                {
                    ...
                    if (min_cnt > (*i).item().cnt)
                    { // found less used entry
                        min_entry = &(*i);
                        min_cnt = (*i).item().cnt;
                    }
                }
...
public:
...

    {
        ...
        if (varinfo)
            strcpy(OTL_RCAST(char*, var_info), varinfo);
...
public:
    ...
    void set_max_long_size(const int amax_size)
    {
        reset_throw_count();
#if defined(OTL_UNICODE)
#if defined(OTL_ORA8I)||defined(OTL_ORA9I)||defined(OTL_RA10G)
...
public:
     ...
     TVariableStruct* var = 0)
    {
        ...
        if (this->adb && this->adb->throw_count > 1)
            return ;
#if defined(OTL_STL) && defined(OTL_UNCAUGHT_EXCEPTION_ON)
...
public:
        ...
        if (rc == SQL_SUCCESS_WITH_INFO || rc == SQL_SUCCESS)
        {
            ...
            else
            {
                if (ftype == otl_var_raw_long)
                    ...
                    chunkLen = retLen / sizeof(OTL_CHAR);
            }
#if defined(OTL_UNICODE)
     ...
     const int /*apl_tab_size*/ = 0)
    {
        ...
        byte_size = aelem_size * OTL_SCAST(unsigned, aarray_size);
#if defined(OTL_UNICODE)
    ...
    void set_len(int len, int ndx)
    {
        ...
        if (ftype == otl_var_char)
            ...
            if (ftype == otl_var_varchar_long)
                p_len[ndx] = OTL_SCAST(OTL_SQLINTEGER, len * sizeof(OTL_CHAR));
            else
    ...
    int get_len(int ndx)
    {
        if (p_len[ndx] == SQL_NULL_DATA)
            ...
            if (ftype == otl_var_varchar_long || ftype == otl_var_char)
                return p_len[ndx] / sizeof(OTL_CHAR);
            else
                return p_len[ndx];
#else
    ...
    void* val(int ndx, int pelem_size)
    {
        ...
        case otl_var_char:
        case otl_var_varchar_long:
            return (void*)&p_v[(OTL_SCAST(unsigned, ndx))*pelem_size*sizeof(OTL_CHAR)];
        default:
        ...
        if (mode != otl_lob_stream_write_mode)
    {
        ...
        char var_info[256];
            ...
            if (var_info[0] != 0)
                vinfo = &var_info[0];
            if (this->connect)
                this->connect->throw_count++;
            if (this->connect && this->connect->throw_count > 1)
                return *this;
#if defined(OTL_STL) && defined(OTL_UNCAUGHT_EXCEPTION_ON)
        ...
        else if ((*ss))
    {
            ...
            if (shell && shell->lob_stream_flag)
        {
            if (this->adb && *this->adb)
                    (*this->adb)->throw_count++;
                if (this->adb && *this->adb && (*this->adb)->throw_count > 1)
                    return 0;
                char* stm_label = 0;
                char* stm_text = 0;
                if ((*io))
...
int eof(void) OTL_NO_THROW
    ...
    otl_stream& operator>>(unsigned char* s)
        ...
        OTL_TRACE_WRITE(OTL_RCAST(wchar_t*, s),
                        "operator >>",
                        "wchar_t*")
#else
...
OTL_THROWS_OTL_EXCEPTION
{
    ...
    *uc = 0;
    s << OTL_RCAST(const unsigned char*, unitemp_val);
#else
    ...
    int server_detach(void)
    {
        ...
        if (authp != 0)
            OCIHandleFree(OTL_RCAST(dvoid*, authp),
                          OTL_SCAST(ub4, OCI_HTYPE_SESSION));
        if (errhp != 0)
            OCIHandleFree(OTL_RCAST(dvoid*, errhp),
        ...
        if (envhp != 0)
            OCIHandleFree(OTL_RCAST(dvoid*, envhp),
...
public:
...
#if defined(OTL_STL) || defined(USER_DEFINED_STRING_CLASS)
    void get
        (const int pos, OTL_STRING_CONTAINER& s)
    {
        check_pos(pos);
    ...
    void get
        (const int pos, otl_long_string& s)
    {
        check_pos(pos);
