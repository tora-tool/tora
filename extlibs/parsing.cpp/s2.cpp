...
void parseFile(const char* fName, int fd)
{
	...
	// version.
	//
	S2LexerTraits::InputStreamType*    input;
	...
	// if you wish.
	//
	S2LexerTraits::TokenStreamType*	tstream;
...
#endif
	input	= new S2LexerTraits::InputStreamType((const ANTLR_UINT8 *)data.c_str(),
						   ANTLR_ENC_8BIT,
	...
	if (lxr == NULL)
	{
		lxr	    = new S2Lexer(input);	    // javaLexerNew is generated by ANTLR
	}
	else
	{
	...
	//
	tstream = new S2LexerTraits::TokenStreamType(ANTLR_SIZE_HINT, lxr->get_tokSource());

	// Finally, now that we have our lexer constructed, we can create the parser
	//
	psr = new S2Parser(tstream);  // javaParserNew is generated by ANTLR3
	S2Parser::start_rule_return r = psr->start_rule();
	...
	{
		ANTLR_INT32 T = 0;
		while	(T != S2Lexer::EOF_TOKEN)
		{
			T = tstream->_LA(1);
			S2LexerTraits::CommonTokenType const* token = tstream->_LT(1);
