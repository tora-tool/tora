...
void parseFile(const char* fName, int fd)
{
	...
	// version.
	//
	S1LexerTraits::InputStreamType*    input;
	...
	// if you wish.
	//
	S1LexerTraits::TokenStreamType*	tstream;
...
#endif
	input	= new S1LexerTraits::InputStreamType((const ANTLR_UINT8 *)data.c_str(),
						   ANTLR_ENC_8BIT,
	...
	if (lxr == NULL)
	{
		lxr	    = new S1Lexer(input);	    // javaLexerNew is generated by ANTLR
	}
	else
	{
	...
	//
	tstream = new S1LexerTraits::TokenStreamType(ANTLR_SIZE_HINT, lxr->get_tokSource());

	// Finally, now that we have our lexer constructed, we can create the parser
	//
	psr = new S1Parser(tstream);  // javaParserNew is generated by ANTLR3
	...
	{
		ANTLR_INT32 T = 0;
		while	(T != S1Lexer::EOF_TOKEN)
		{
			T = tstream->_LA(1);
			S1LexerTraits::CommonTokenType const* token = tstream->_LT(1);
