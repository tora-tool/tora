...
void parseFile(const char* fName, int fd)
{
	...
	// version.
	//
	OraclePLSQLLexerTraits::InputStreamType*    input;
	...
	// if you wish.
	//
	OraclePLSQLLexerTraits::TokenStreamType*	tstream;
...
#endif
	input	= new OraclePLSQLLexerTraits::InputStreamType((const ANTLR_UINT8 *)data.c_str(),
						   ANTLR_ENC_8BIT,
	...
	if (lxr == NULL)
	{
		lxr	    = new OraclePLSQLLexer(input);	    // javaLexerNew is generated by ANTLR
	}
	else
	{
	...
	//
	tstream = new OraclePLSQLLexerTraits::TokenStreamType(ANTLR_SIZE_HINT, lxr->get_tokSource());

	// Finally, now that we have our lexer constructed, we can create the parser
	//
	psr = new OraclePLSQLParser(tstream);  // javaParserNew is generated by ANTLR3
	psr->start_rule();
	...
	{
		ANTLR_INT32 T = 0;
		while	(T != OraclePLSQLLexer::EOF_TOKEN)
		{
			T = tstream->_LA(1);
			OraclePLSQLLexerTraits::CommonTokenType const* token = tstream->_LT(1);
